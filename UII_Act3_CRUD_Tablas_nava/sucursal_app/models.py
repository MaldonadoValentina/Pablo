...
class Sucursal(models.Model):
    ...
    numero_telefono = models.CharField(
        max_length=15,
        validators=[RegexValidator(r'^\+?1?\d{9,15}$', message="El número debe ser entre 9 y 15 dígitos, y puede incluir '+' al inicio.")])
...
class Provedor(models.Model):
    ...
    numero_telefono_provedor = models.CharField(
            max_length=15,
            validators=[RegexValidator(r'^\+?1?\d{9,15}$', message="El número debe ser entre 9 y 15 dígitos, y puede incluir '+' al inicio.")])
